...
<html lang="en">
    ...
    <body>
        ...
        <%
            ...
            String nomeUt = (String) request.getAttribute("nomeU");
            if ((session == null) || (!session.getAttribute("name").toString().equalsIgnoreCase(nomeUt))) {
                        ...
                        <form class="navbar-form " role="search">
                            <div id="nav-centrata" class="input-group" >
                                ...
                                <span class="input-group-btn">
                                    <button class="btn mybtn-primary height-40" type="button" onclick="parseSend()"><fmt:message key="searchButton"/></button>
                                </span>
                ...
                <div class="collapse navbar-collapse hidden-lg" id="search">
                    <form class="mobile_search hidden-sm hidden-md hidden-lg" role="search" action="/" method="GET">
                        <div class="input-group">
                            ...
                            <span class="input-group-btn">
                                <a href="#" class="btn mybtn-primary" type="button"><fmt:message key="searchButton"/></a>
                            </span>
        ...
        <div class="row">
            ...
            <div class="col-xs-12- col-sm-12 col-md-2 col-lg-2">
                ...
                <div class="list-group">
                    ...
                    %>        
                    <a onclick="doHref($(this).children('.nascosto').attr('value'))" class=" categorie puntatore list-group-item"> <%=selCat.getNome()%>
                        <div class="nascosto hidden" value="ActionServlet?op=getList2&filtro=<%=selCat.getNome()%>&search=<%=key%>&tag=<%=tag%>&nomeU=<%=nome_utente%>"> </div> </a>
                    <%
                        }
                    %>
                </div>
                    ...
                    <div class="row">
                                    ...
                                    <div class="pull-left">
                                        ...
                                        <h4>
                                            <%
                                                if (key != null) {
                                            %> 
                                            <div class="mybreadcrumb"> <%= servicesDim%> <fmt:message key="msgSearch"/> </div>
                                                <%
                                                    }
                                                    if ((filter != null)) {
                                                %>   
                                            <div class="mybreadcrumb" onmouseover="showXFilter()" onmouseout="hideXFilter()">
                                                <fmt:message key="categoryBread" /> <%= filter + " "%>  <div class="xBread" onclick="doHref('ActionServlet?op=getList2&nomeU=<%=nome_utente%>&search=<%=key%>&tag=<%=tag%>&orderBy=<%=ordinamento%>')" ><span class="glyphicon delete glyphicon-remove-circle hidden" 
                                                                                                                                                                        id="filterBreadX"></span> </div>
                                            </div>                           
                                            <%
                                                }
                                            ...
                                            %>
                                            <div class="mybreadcrumb" onmouseover="showXTag()" onmouseout="hideXTag()">
                                                <fmt:message key="tagBread"/>  #<%= tag%>   <div  class="xBread" onclick="doHref('ActionServlet?op=getList2&nomeU=<%=nome_utente%>&search=<%=key%>&filtro=<%=filter%>&orderBy=<%=ordinamento%>')" > <span class="glyphicon delete glyphicon-remove-circle hidden"  id="tagBreadX"></span> </div> </div>
                                            <%
                                                }
                                            ...
                                            <%
                                                ...
                                                if (key != null) {
                                            %>
                                            <div class="mybreadcrumb text-primary" onmouseover="showXKey()" onmouseout="hideXKey()"> 
                                                "<%= searchBar.trim()%>"  <div  class="xBread" onclick="doHref('ActionServlet?op=getList2&nomeU=<%=nome_utente%>&tag=<%=tag%>&filtro=<%=filter%>&orderBy=<%=ordinamento%>')"> <span class="glyphicon delete glyphicon-remove-circle hidden" 
                                                                                                                                                                                                     id="keyBreadX"></span> </div> </div>
                                                    <%
                                                        }
                                                    %>
                                ...
                                <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                    <div id="orderByDiv" class="pull-right">
                                        <label for="sel1" class="mylabel"> <fmt:message key="orderButton" /> </label> <select id="sel1" class="myselect mybtn-primary"
                                            ...
                                            <option id="ordNome" class="myoption" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&filtro=<%=filter%>&search=<%=key%>&tag=<%=tag%>&orderBy=nome">  
                                                <fmt:message key="ordName"/>  </option> 
                                            <option id="ordUsiMax" class="myoption" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&filtro=<%=filter%>&search=<%=key%>&tag=<%=tag%>&orderBy=utilizziMax"> 
                                                <fmt:message key="ordUsiMax"/> </option>
                                            <option id="ordUsiMin" class="myoption" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&filtro=<%=filter%>&search=<%=key%>&tag=<%=tag%>&orderBy=utilizziMin">   
                                                <fmt:message key="ordUsiMin"/> </option>
                                            <option id="ordRatMax" class="myoption" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&filtro=<%=filter%>&search=<%=key%>&tag=<%=tag%>&orderBy=votoMax">   
                                                <fmt:message key="ordRatMax"/></option>
                                            <option id="ordRatMin" class="myoption" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&filtro=<%=filter%>&search=<%=key%>&tag=<%=tag%>&orderBy=votoMin">  
                                                <fmt:message key="ordRatMin"/></option>
                                        </select>
                                    </div>
                                </div> 
                            </div>
                        ...
                        <%
                            ...
                            while (iterServ.hasNext()) {
                                ...
                                Set<Category> catServiceList = service.getCategory();
                                Iterator iterTag = taglist.iterator();
                                Iterator catTag = catServiceList.iterator();
                        %>
                        <div class="row">
                            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 mywell" id="DIV<%= service.getId()%>">
                                ...
                                <div class="col-sm-12 col-md-5 col-lg-7">
                                    <h4 class="title"> <b> <%=service.getNome()%> </b> </h4>
                                    <h6 class="title"> <fmt:message key="byDiv"/> <%=service.getNome()%> </h6>
                                    <h5 class="description"> 
                                        <fmt:message key="descriptionDiv"/>
                                        <%=service.getDescrizione()%>
                                    </h5>
                                    <div class="div-menu">
                                        <fmt:message key="categoryDiv"/>
                                        <%
                                            while (catTag.hasNext()) {
                                                Category selCat = (Category) catTag.next();
                                        %>

                                        <div class="a-button catButton" onclick="doHref($(this).children('.nascosto').attr('value'))">  <%= selCat.getNome()%> 
                                            <div class="nascosto hidden" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&search=<%=key%>&filtro=<%=selCat.getNome()%>&orderBy=<%=ordinamento%>&tag=<%=tag%>"> </div> </div>
                                    ...
                                    <div class="div-menu">
                                        <fmt:message key="tagDiv"/>
                                        <%
                                            while (iterTag.hasNext()) {
                                                Tag selTag = (Tag) iterTag.next();
                                        %>

                                        <div class="a-button tagButton" onclick="doHref($(this).children('.nascosto').attr('value'))"> #<%= selTag.getNome()%> 
                                            <div class="nascosto hidden" value="ActionServlet?op=getList2&nomeU=<%=nome_utente%>&search=<%=key%>&filtro=<%=filter%>&tag=<%=selTag.getNome()%>&orderBy=<%=ordinamento%>"></div></div>
                                ...
                                <div class="col-sm-12 col-md-3 col-lg-3 votiBottoni">
